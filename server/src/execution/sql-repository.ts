...
function toRecord(r: ExecutionRow): ExecutionRecord {
  ...
  return {
    ...
    ...(r.command_intent ? { commandIntent: dialect.json<ExecutionRecord['commandIntent']>(r.command_intent) } : {}),
    ...(typeof r.command_hash === 'string' ? { commandHash: r.command_hash } : {}),
...
function toValues(rec: ExecutionRecord): unknown[] {
  ...
  return [
    ...
    jsonParam(rec.commandIntent),
    rec.commandHash ?? null,
