...
1. **KISS，不 over-design**：用最简单的实现满足当前需求。不预留“以后可能用”的抽象、配置项、分支；不引入第二套 DSL / runtime / 端点 / 表，能复用 `execution` / HumanTask / `proposeAction` 就复用。不在业务层写方言分支，不在路由里写 `if provider===`。
