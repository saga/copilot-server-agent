...
export const api = {
  ...
  models(): Promise<{ models: unknown }> {
    return fetch(`${API_BASE}/api/models`).then(json<{ models: unknown }>);
  },
  ...
  chatStream(
    sessionId: string,
    prompt: string,
    callbacks: { onDelta: (d: string) => void; onDone?: () => void; onError?: (e: Error) => void },
    model?: string,
  ): void {
